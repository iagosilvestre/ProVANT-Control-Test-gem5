						           ...
						           std::vector<double>& y, std::vector<int>& I)
{
	...
	// Get sensor data plus noise (indexes from UniversalLinkSensor.cpp and UniversalJointSensor.cpp)
	// GPS
	double xB = mainbody.values.at(24) + GPS_x_noise;
	double yB = mainbody.values.at(25) + GPS_y_noise;
	
	// Barometer
	double zB = mainbody.values.at(26) + Barometer_noise;
	
	// IMU
	double phiB   = mainbody.values.at(27) + IMU_phi_noise;
	double thetaB = mainbody.values.at(28) + IMU_theta_noise;
	double psiiB  = mainbody.values.at(29) + IMU_psii_noise;
	std::vector<double> pqr = wIIL2pqr(mainbody.values.at(39), mainbody.values.at(40), mainbody.values.at(41),
                                           mainbody.values.at(27), mainbody.values.at(28), mainbody.values.at(29));
...
//	double dA1A1L_z = dA1A1L(2) + Camera_z(gen11);
	...
	// Servos
	double aR = rightservo.values.at(0) + Servos_aR_noise;
	double aL = leftservo.values.at(0) + Servos_aL_noise;
	double aRdot = rightservo.values.at(1) + Servos_daR_noise;
	double aLdot = leftservo.values.at(1) + Servos_daL_noise;
