/**
  ******************************************************************************
  * @file    modules/control/c_control_receiver.c
...
/* Includes ------------------------------------------------------------------*/
#include "c_control_BS.h"

/** @addtogroup Module_control
  * @{
  */

/** @addtogroup Module_control_Component_Control
  * \brief Controle de estabilização para o modo de operação control.
  *
  ...
  *
  * while(1) {
  *   	actuation = control_controller(attitude,
  * 					attitude_reference,
  * 					position,
  * 					position_reference);
...
float32_t c_control_BS_AH_altitude_controller_step(float altitude, float altitude_reference, float rateOfClimb, float rateOfClimb_reference,
									pv_type_datapr_attitude attitude);
arm_matrix_instance_f32 c_control_BS_AH_PD_gains_step(pv_type_datapr_attitude attitude, pv_type_datapr_attitude attitude_reference, bool enable_integrators);
arm_matrix_instance_f32 c_control_BS_AH_torque_calculation_step(pv_type_datapr_attitude attitude, arm_matrix_instance_f32 gamma);
pv_type_actuation c_control_BS_AH_actuators_signals_step(arm_matrix_instance_f32 tau, float Fzb);
arm_matrix_instance_f32 c_control_BS_AH_inertia_matrix(pv_type_datapr_attitude attitude);
arm_matrix_instance_f32 c_control_BS_AH_coriolis_matrix(pv_type_datapr_attitude attitude);
void c_control_BS_AH_integrate_error(pv_type_stability_error current_error, float sample_time);
float c_control_BS_AH_Fzb_control(float signal_control);

//pv_msg_io_actuation LQR(arm_matrix_instance_f32 state_vector);

/* Private functions ---------------------------------------------------------*/

// rateOfClimb (RoC) = aicontrolraft's vertical speed (velocity in Z-axis)
float c_control_BS_AH_altitude_controller_step(float altitude, float altitude_reference, float rateOfClimb, float rateOfClimb_reference, pv_type_datapr_attitude attitude)
{
	float Zeta3=0;
...
float c_control_BS_AH_Fzb_control(float throttle_control){
...
float c_control_integrate_trapezoidal(float last_integration, float current_value, float last_value, float sample_time){
 ...
 */
void c_control_BS_AH_integrate_error(pv_type_stability_error error, float sample_time){

	#ifdef ENABLE_INT_ROLL
//		BS_AH_integrated_error.roll = c_control_saturation( c_control_integrate_trapezoidal(BS_AH_integrated_error.roll, error.roll, BS_AH_last_error.roll, sample_time), INT_ROLL_LOWER_ER_LIMIT, INT_ROLL_UPPER_ER_LIMIT);
	BS_AH_integrated_error.roll = c_control_integrate_trapezoidal(BS_AH_integrated_error.roll, error.roll, BS_AH_last_error.roll, sample_time);
		BS_AH_last_error.roll  = error.roll;
	#endif
	#ifdef ENABLE_INT_PITCH
//		BS_AH_integrated_error.pitch = c_control_saturation( c_control_integrate_trapezoidal(BS_AH_integrated_error.pitch, error.pitch, BS_AH_last_error.pitch, sample_time), INT_PITCH_LOWER_ER_LIMIT, INT_PITCH_UPPER_ER_LIMIT);
		BS_AH_integrated_error.pitch = c_control_integrate_trapezoidal(BS_AH_integrated_error.pitch, error.pitch, BS_AH_last_error.pitch, sample_time);
		BS_AH_last_error.pitch = error.pitch;
	#endif
	#ifdef ENABLE_INT_YAW
		BS_AH_integrated_error.yaw = c_control_saturation( c_control_integrate_trapezoidal(BS_AH_integrated_error.yaw, error.yaw, BS_AH_last_error.yaw, sample_time), INT_YAW_LOWER_ER_LIMIT, INT_YAW_UPPER_ER_LIMIT);
		BS_AH_last_error.yaw   = error.yaw;
	#endif
	#ifdef ENABLE_INT_Z
//		BS_AH_integrated_error.z = c_control_saturation( c_control_integrate_trapezoidal(BS_AH_integrated_error.z, error.z, BS_AH_last_error.z, sample_time), INT_Z_LOWER_ER_LIMIT, INT_Z_UPPER_ER_LIMIT);
		BS_AH_integrated_error.z = c_control_integrate_trapezoidal(BS_AH_integrated_error.z, error.z, BS_AH_last_error.z, sample_time);
...
arm_matrix_instance_f32 c_control_BS_AH_PD_gains_step(pv_type_datapr_attitude attitude, pv_type_datapr_attitude attitude_reference, bool enable_integrators){
	...
	if (enable_integrators)
		c_control_BS_AH_integrate_error(current_error, CONTROL_SAMPLE_TIME);
	else{
...
arm_matrix_instance_f32 c_control_BS_AH_torque_calculation_step(pv_type_datapr_attitude attitude, arm_matrix_instance_f32 gamma)
{
	...
	inr_att = c_control_BS_AH_inertia_matrix(attitude);
	cor_att = c_control_BS_AH_coriolis_matrix(attitude);
				  ...
				  bool enable_integration)
{
	...
	if (enable_integration)
		gamma = c_control_BS_AH_PD_gains_step(attitude, attitude_reference, true);
	else
		gamma = c_control_BS_AH_PD_gains_step(attitude, attitude_reference, false);

	tau = c_control_BS_AH_torque_calculation_step(attitude, gamma);

	if (!manual_height_control)
		Fzb = c_control_BS_AH_Fzb_control(throttle_control);
	else{

		Fzb = c_control_BS_AH_altitude_controller_step(position.z, position_reference.z, position.dotZ, position_reference.dotZ, attitude);

	}
	actuation_signals = c_control_BS_AH_actuators_signals_step(tau, Fzb);
