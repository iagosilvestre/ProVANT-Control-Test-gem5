...
#define betaInit			1.5f        // 2 * proportional gain

//---------------------------------------------------------------------------------------------------
// Variable definitions

float volatile beta = betaInit;								// 2 * proportional gain (Kp)
...
/** \brief Troca o ganho do estimador por seu valor operacional
 * O ganho beta do estimador comeca com um valor alto para convergir mais rapido. Esta funcao diminui tal valor para o valor operacional.
 */
void c_datapr_setBetaOperational(){
	beta = betaOperational;
}

//---------------------------------------------------------------------------------------------------
// AHRS algorithm update

void c_datapr_MadgwickAHRSupdate(float * q,float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
	float recipNorm;
	float s0, s1, s2, s3;
	...
	if((mx == 0.0f) && (my == 0.0f) && (mz == 0.0f)) {
		c_datapr_MadgwickAHRSupdateIMU(q, gx, gy, gz, ax, ay, az);
		return;
	}
	...
	if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
		...
		// From http://diydrones.com/forum/topics/madgwick-imu-ahrs-and-fast-inverse-square-root?id=705844%3ATopic%3A1018435&page=4#comments
...
void c_datapr_MadgwickAHRSupdateIMU(float * q,float gx, float gy, float gz, float ax, float ay, float az) {
	float recipNorm;
	float s0, s1, s2, s3;
	float qDot1, qDot2, qDot3, qDot4;
