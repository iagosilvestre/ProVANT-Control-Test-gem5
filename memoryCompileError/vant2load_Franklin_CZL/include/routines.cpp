						           ...
						           std::vector<double>& y, std::vector<int>& I)
{
	...
	// GPS
	double xB = gem5 + GPS_x(gen00);
	double yB = gem5 + GPS_y(gen01);
	
	// Barometer
	double zB = gem5 + Barometer(gen02);
	
	// IMU
	double phiB   = gem5 + IMU_phi(gen03);
	double thetaB = gem5 + IMU_theta(gen04);;
	double psiiB  = gem5 + IMU_psii(gen05);;
	std::vector<double> pqr = wIIL2pqr(gem5, gem5, gem5,
			gem5, gem5, gem5);
...
//	double dA1A1L_z = dA1A1L(2) + Camera_z(gen11);
	...
	// Servos
	double aR = gem5 + Servos_aR(gen12);
	double aL = gem5 + Servos_aL(gen13);
	double aRdot = gem5 + Servos_daR(gen14);
	double aLdot = gem5 + Servos_daL(gen15);
