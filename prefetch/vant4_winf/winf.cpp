...
//			      0,     0,     0,   100;
	...
	public: std::vector<double> execute(simulator_msgs::SensorArray arraymsg)
	{
		...
		//Variáveis de Configuração
                double AlphaR = (notrandom);
                double AlphaL = (notrandom);
                ...
                //Derivada Variáveis de Configuração
                double AlphaRp = (notrandom);
                double AlphaLp = (notrandom);
                double Phip = (notrandom);
                double Thetap = (notrandom);
                double Psip = (notrandom);
                double Xp = (notrandom);
                double Yp = (notrandom);
                double Zp = (notrandom);
...
//		}
		...
		double x_error = (notrandom)-Trajectory(1);
		xint = xint + (T/2)*(x_error + x_ant);
		x_ant = x_error;

		double y_error = (notrandom)-Trajectory(2);
		yint = yint + (T/2)*(y_error + y_ant);
		y_ant = y_error;

        	double z_error = (notrandom)-Trajectory(3);
