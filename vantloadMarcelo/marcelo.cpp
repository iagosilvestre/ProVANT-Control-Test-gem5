...
//		}
		...
		double x_atual =(rand() % 10) - Ref(0);
		xint = xint + (T/2)*(x_atual + x_ant);
		x_ant = x_atual;
		double y_atual = (rand() % 10) - Ref(1);
		yint = yint + (T/2)*(y_atual + y_ant);
		y_ant = y_atual;
		double z_atual = (rand() % 10) - Ref(2);
		zint = zint + (T/2)*(z_atual + z_ant);
		z_ant = z_atual;
		double yaw_atual = (rand() % 10) - Ref(3);
		yawint = yawint + (T/2)*(yaw_atual + yaw_ant);
		yaw_ant = yaw_atual;

		

		std::vector<double> etadot = wIIL2EtaDot((rand() % 10),
				(rand() % 10),
				(rand() % 10),
				(rand() % 10),
				(rand() % 10),
