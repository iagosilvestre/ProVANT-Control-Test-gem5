						           ...
						           std::vector<double>& y, std::vector<int>& I)
{
	...
	// GPS
	double xB = (auxDeb) + GPS_x(gen00);
	double yB = (auxDeb) + GPS_y(gen01);
	
	// Barometer
	double zB = (auxDeb) + Barometer(gen02);
	
	// IMU
	double phiB   = (auxDeb) + IMU_phi(gen03);
	double thetaB = (auxDeb) + IMU_theta(gen04);;
	double psiiB  = (auxDeb) + IMU_psii(gen05);;
	std::vector<double> pqr = wIIL2pqr((auxDeb), (auxDeb), (auxDeb),
			(auxDeb), (auxDeb), (auxDeb));
...
//	double dA1A1L_z = dA1A1L(2) + Camera_z(gen11);
        ...
        double gamma1 = (auxDeb);
        double gamma2 = (auxDeb);
	...
	// Servos
	double aR = (auxDeb) + Servos_aR(gen12);
	double aL = (auxDeb) + Servos_aL(gen13);
	double aRdot = (auxDeb) + Servos_daR(gen14);
	double aLdot = (auxDeb) + Servos_daL(gen15);
